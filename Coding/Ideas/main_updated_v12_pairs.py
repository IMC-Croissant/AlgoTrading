...
class Trader:
    ...
    def pairs_quantity(self, state: TradingState, products: list):
        current_volume = 0
        pina_vol = 0
        coco_vol = 0
        if bool(state.position): 
            if products[0] in state.position.keys():
                pina_vol = state.position[products[0]]
            if products[1] in state.position.keys():
                coco_vol = state.position[products[1]]
        l1_bid_pina, l1_ask_pina = self.get_l1(state, products[0])
        pina_avail_bid = state.order_depths[products[0]].buy_orders[l1_bid_pina]
        pina_avail_ask = state.order_depths[products[0]].sell_orders[l1_ask_pina]

        l1_bid_coco, l1_ask_coco = self.get_l1(state, products[1])
        coco_avail_bid = state.order_depths[products[1]].buy_orders[l1_bid_coco]
        coco_avail_ask = state.order_depths[products[1]].sell_orders[l1_ask_coco]

        # gives long pair and short pair available buy and sell
        long_pair_pos = min(pina_avail_ask, math.floor(coco_avail_bid/2), 200)
        short_pair_pos = min(pina_avail_bid, math.floor(coco_avail_ask/2), 200)


        max_long_position = self.product_limits[products[0]] - current_volume
        max_short_position = -self.product_limits[products[1]] - current_volume

        return long_pair_pos, short_pair_pos
...
## --------------------------- End Pairs ---------------------- ## 
    ...
    def _process_new_data(self, state: TradingState) -> None:
        ...
        for product in state.order_depths.keys():
            min_ask = min(state.order_depths[product].sell_orders.keys())
            max_bid = max(state.order_depths[product].buy_orders.keys())
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            if product == 'PINA_COLADAS' or product == 'COCONUTS': 
                ...
                pina_long_quant, pina_short_quant = self.pairs_quantity(state, ['PINA_COLADAS', 'COCONUTS'])
                coco_long_quant, coco_short_quant = self.pairs_quantity(state, ['PINA_COLADAS', 'COCONUTS'])
                coco_long_quant, coco_short_quant = math.floor(1.87*2*coco_long_quant),math.floor(1.87*2*coco_short_quant)
                ...
                if trade_pairs == True and (self.trade_active == 'Neutral' or self.trade_active == 'Short Pair'):   
                    orders.append(Order('PINA_COLADAS', pina_ask, pina_long_quant))
                    orders.append(Order('COCONUTS', coco_bid, -coco_long_quant))
                ...
                # check if we must close trade
                    ...
                    if manage_pairs: # we have to close trade
                        orders.append(Order('PINA_COLADAS', pina_bid, -state.position['PINA_COLADAS'])) # SELL OUR PINAS at bid
                        orders.append(Order('COCONUTS', coco_bid, -state.position['COCONUTS'])) # BUY OUR COCOS at ask
                ...
                # check if we are short a pair already, if we are not short a pair and have a signal, we short!       
                    ...
                    orders.append(Order('PINA_COLADAS', pina_bid, -pina_short_quant))
                    orders.append(Order('COCONUTS', coco_ask, coco_short_quant))
                ...
                # check if we must close trade
                    ...
                    if manage_pairs: # we have to close trade if true
                        orders.append(Order('PINA_COLADAS', pina_ask, -state.position['PINA_COLADAS'])) # BUY OUR PINAS at ask
                        orders.append(Order('COCONUTS', coco_bid, -state.position['COCONUTS'])) # SELL OUR COCOS at bid
                        self.trade_active == 'Neutral'
            # else:

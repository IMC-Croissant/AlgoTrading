...
class Trader:
    ...
    _previous_buy_indicator = {'PINA_COLADAS': False, 'COCONUTS': False}
    _sell_indicator = {'PINA_COLADAS': False, 'COCONUTS': False}
    _previous_sell_indicator = {'PINA_COLADAS': False, 'COCONUTS': False}
    ...
    def _get_ewm_values_and_indicator(self, state: TradingState, product: str) -> bool:
        ...
        def sma_cross_indicator(self, state: TradingState, product: str, history_product, sma_low_time, sma_high_time) -> None:
            """
            Calculate SMA
            """
            sma_low, sma_high = -1, -1
            if state.timestamp > sma_high_time * 100:
                sma_low = history_product.iloc[-sma_low_time:].mean()
                sma_high = history_product.iloc[-sma_high_time:].mean()
        ...
        def ewm_cross_indicator(self, signal, macd) -> None:
            """
            Calculate EWM
            """
            if signal != -1 and macd != -1:
                if signal < macd:
                    self._sell_indicator[product] = False
                    self._buy_indicator[product] = True
                elif signal > macd:
        ...
        if product == 'PINA_COLADAS':
            ...
            sma_cross_indicator(self, state, product, history_product, 50, 200)
    ...
    def _get_acceptable_price(
        ...
        if product == "PEARLS":
            ...
            #if ratio > 1 and spread < 4:
            #    fair_value += 3
            #elif ratio < 1 and spread < 4:
            #    fair_value -= 3
            if self._buy_indicator[product]:
                acceptable_bid = fair_value -1
            if self._sell_indicator[product]:

...
## --------------------------- Start Pairs ---------------------- ## 
    ...
    #    closeTrade = False
    #    if (self.trade_active == 'Long Pair') and (self.ratio_g >= self._pc_ratio_mean - 1.8* self._pc_ratio_std): # we are currently long a pair
    #        # must close trade
    #        closeTrade = True
    #    elif self.trade_active == 'Short Pair' and (self.ratio_g <= self._pc_ratio_mean + 1.8 * self._pc_ratio_std):
    #        # must close trade
    #        closeTrade = True
    #    return closeTrade
...
## --------------------------- End Pairs ---------------------- ## 
    ...
    def _get_acceptable_quantity(
        ...
        #print("acceptable buy vol {} sell vol {} product {}".format(buy_volume, sell_volume, product))
    ...
    def _get_acceptable_price(
        ...
        #print("acceptable bid {} ask {} product {}".format(acceptable_bid, acceptable_ask, product))
    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        ...
        for product in state.order_depths.keys():
            ...
            fair_prices, bullish = self._get_ewm_values_and_indicator(
                    state, product)
            # pairs trading
            if product == 'PINA_COLADAS': # or product == 'COCONUTS':  only have to do once for pina colads and coconut
                ...
                if (state_position_fixed['COCONUTS'] + 2*short_pair_quant >= 600) or \
                    ...
                    short_pair_quant = min(abs(-300-state_position_fixed['PINA_COLADAS']), abs(math.ceil((-600-state_position_fixed['COCONUTS'])/2)), short_pair_quant)
                ...
                if trade_pairs == True: #and (self.trade_active == 'Neutral' or self.trade_active == 'Short Pair'): l1 from active trade might not be able to fill trade
                    ...
                    if (state_position_fixed['PINA_COLADAS'] +long_pair_quant > 0) and (state_position_fixed['COCONUTS'] - short_pair_quant*2 < 0):
                ...
                # check if we are short a pair already, if we are not short a pair and have a signal, we short!       
                    ...
                    if (state_position_fixed['PINA_COLADAS'] - short_pair_quant < 0) and (state_position_fixed['COCONUTS'] + short_pair_quant*2 > 0):
